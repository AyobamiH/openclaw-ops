...
export const INITIAL_WIKI_FEED = JSON.stringify({
...
/** Shared setup: seed Redis defaults, initialize wiki, start scheduler. */
async function initWikiAndScheduler(): Promise<void> {
  ...
  // Always (re)write the wiki page so the scheduler has data to read
  try {
    await reddit.updateWikiPage({
      subredditName: context.subredditName,
      page: 'milestones-feed',
      content: INITIAL_WIKI_FEED,
      reason: 'Initialized/refreshed by openclawdbot',
    });
...
triggers.post('/on-app-upgrade', async (c) => {
  try {
    ...
    await initWikiAndScheduler();
    return c.json<TriggerResponse>({ status: 'success', message: 'App upgraded — wiki + scheduler refreshed' }, 200);
  } catch (error) {
    console.error('[triggers] upgrade error:', error);
    return c.json<TriggerResponse>({ status: 'error', message: 'Upgrade handler failed' }, 400);
  }
});
