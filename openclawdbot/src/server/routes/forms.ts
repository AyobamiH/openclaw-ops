...
forms.post('/milestone-secret-submit', async (c) => {
  ...
  if (!trimmed) {
    return c.json<UiResponse>({ showToast: 'Secret cannot be empty.' }, 200);
  }
...
forms.post('/milestone-feed-url-submit', async (c) => {
  ...
  if (!trimmed) {
    return c.json<UiResponse>({ showToast: 'Feed URL cannot be empty.' }, 200);
  }
  if (!trimmed.startsWith('https://')) {
    return c.json<UiResponse>({ showToast: 'Feed URL must start with https://' }, 200);
  }
