...
- Run sync_openai_cookbook.sh manually to fetch cookbook
- Trigger drift-repair task to generate knowledge pack (processor now reads both paths)
- Verify pack contains docs from both sources with correct source tags
- Test reddit-helper loads and uses mixed-source pack correctly
- Verify LLM responses credit appropriate sources when answering

### Current State
- ✅ Sync infrastructure ready
- ✅ Config updated
- ✅ Doc-specialist processor updated to dual-source
- ✅ Reddit-helper updated to handle source-tagged docs
- ⏳ Integration test (sync + process + use) — waiting for next session or manual trigger
